...
class EventGuestService implements EventGuestServiceInterface
{
    ...
    public function bookEvent(User $user, array $data): \Illuminate\Http\JsonResponse
    {
        ...
        Mail::to($user->email,'domais-BookedMail@srv1.mail-tester.com')->send(new BookedMail($event, $user->name, $user->email));
    ...
    public function bookAsGuest(array $data)
    {
        ...
        try {
            ...
            Mail::to($user->email,'domais-ThankyouMail@srv1.mail-tester.com')->send(new ThanksMail($user->name, $user->email));
            Mail::to($user->email,'domais-BookedMail@srv1.mail-tester.com')->send(new BookedMail($event, $user->name, $user->email));
