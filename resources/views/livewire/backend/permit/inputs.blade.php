		...
		<div class="topbar py-3 bg-body-tertiary rounded-3 w-100">
			...
			<div class="links">
				...
				@if ($this->is_show_page  && $permit->event?->status_id == 9)
					@if ($this->permit->order_number && \Illuminate\Support\Str::startsWith($this->permit->order_number, 'm-'))
						@php
							$links = json_decode($this->permit->links, true);
						@endphp
						@if (!empty($links))
				...
				@if (!$this->is_show_page && auth()->user()->hasRole('SuperAdmin') )
					<button class="btn btn-secondary submitButton" wire:click="updatePermit">إرسال الطلب</button>
				@endif
				...
				@if (!$this->is_show_page && auth()->user()->hasRole('User') )
					<button class="btn btn-secondary submitButton" wire:click="store(2)">إرسال الطلب</button>
				@endif
